...
def getWordScore(word, n):
    ...
    returns: int >= 0
    """
    wordlow = word.lower() #convert input to lower case
    list = [ ] #initialize an empty list
    score = 0 #initialize score
    for letters in wordlow: #loop over word
        list.append(SCRABBLE_LETTER_VALUES[letters]) #add value of dictionary to list
    for a in list: #loop over list to add up score
        score += a
    score = score*len(word) #weird scrabble rule
    if n == len(word): #not a weird scrabble rule
        score+=50
...
def updateHand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    copyhand = hand.copy() #create a copy of the dictionary hand
    for letters in word: #iterate over the letters in word
        copyhand[letters] = copyhand[letters]-1 #subtract 1 from the values of the copied dictionary if the letter is found when iterating over word
    hand = copyhand #reassign the value of hand
    return hand    
...
def isValidWord(word, hand, wordList):
    ...
    """
    if word in wordList: #detect if word is in wordlist, if not return false
        ...
        for i in word: #iterate over letters in word
            ...
            if copyhand[i] == -1: #if you don't have enough letters, return False and get out
                return False
                break
        return True #If you pass all of those other commands, you good
    else:
        return False #if your word is not in the wordlist you aint.  
#
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:# As long as there are still letters left in the hand:
        ...
        aGuess = raw_input('Enter word, or a "." to indicate that you are finished: ').lower()# Ask user for input
        
        if aGuess == '.':# If the input is a single period:
    ...
    if calculateHandlen(hand) == 0:
        print 'Run out of letters. Total score: ', score# Game is over (user entered a '.' or ran out of letters), so tell user the total score
